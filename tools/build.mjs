...
function methodPage() {
  return page({
    ...
    content: `<header class="utility-page-header"><div class="shell utility-heading-grid"><div><h1 class="page-title">Check an AI claim against the evidence.</h1><p class="lead">Use four lines to compare a claim with the record, then choose Supported, Contradicted or Insufficient evidence.</p></div><div class="utility-header-side"><div class="utility-actions"><a class="primary-button" href="#quick-check"><span>Use the four-line check</span>${arrowIcon()}</a><a class="secondary-button" href="#questions"><span>Answer common questions</span>${arrowIcon()}</a></div><div class="intent-boundary intent-boundary-compact"><span aria-hidden="true">≠</span><strong>Intent: not assessed</strong></div></div></div></header><nav class="page-jump-nav narrow" aria-label="On this page"><a href="#quick-check">Quick check</a><a href="#questions">Questions</a><a href="#findings">Findings</a><a href="#reference">Reference</a></nav><div class="narrow prose">
      <section class="method-transfer" id="quick-check" aria-labelledby="method-transfer-title"><h2 id="method-transfer-title">Use this four-line record on another AI answer</h2><pre class="method-template"><code>Claim:
Required evidence:
...
Finding: Supported / Contradicted / Insufficient evidence — Intent: not assessed</code></pre><div class="method-transfer-actions"><a class="primary-button" href="../cases/unsupported-citation/">Practice the template on Case 03</a><a class="secondary-button" href="../cases/">Browse all six cases</a></div></section>
      <h2 id="questions">Common questions about checking AI answers</h2>
...
function challengePage() {
  ...
  return page({
    ...
    content: `<header class="utility-page-header"><div class="shell utility-heading-grid"><div><h1 class="page-title">The record should be challengeable.</h1><p class="lead">Choose the route that matches what you found. Keep submissions public-safe; local reproduction is optional.</p></div><div class="utility-actions"><a class="primary-button" href="#evidence-routes"><span>Choose an evidence route</span>${arrowIcon()}</a><a class="secondary-button" href="#local-checker"><span>Run the optional checker</span>${arrowIcon()}</a></div></div></header><div class="narrow prose"><h2 id="evidence-routes">Choose an evidence route</h2><ul class="route-list"><li><a class="route-link" href="${issueBase}counterexample.yml"><span class="route-index">01</span><span><strong>I found evidence that changes a result</strong><span class="route-term">Counterexample</span>Show evidence that changes a case classification or boundary.</span><span>Issue template</span></a></li><li><a class="route-link" href="${issueBase}reproduction.yml"><span class="route-index">02</span><span><strong>I reproduced a result</strong><span class="route-term">Reproduction result</span>Share an exact command, revision and public-safe output.</span><span>Issue template</span></a></li><li><a class="route-link" href="${issueBase}new-case.yml"><span class="route-index">03</span><span><strong>I want to propose a synthetic case</strong><span class="route-term">New synthetic case</span>Propose an issue-first claim and evidence record.</span><span>Issue template</span></a></li><li><a class="route-link" href="${issueBase}accessibility.yml"><span class="route-index">04</span><span><strong>I found an accessibility or site problem</strong><span class="route-term">Accessibility or site defect</span>Report a reproducible barrier without sensitive data.</span><span>Issue template</span></a></li></ul><h2 id="local-checker">Optional local checker</h2><pre class="code-block"><code>git clone https://github.com/TheDarkniteFalls/detecting-ai-deception.git
...
function aboutPage() {
  return page({
    ...
    content: `<header class="utility-page-header"><div class="shell utility-heading-grid"><div><h1 class="page-title">Inspect how every result was produced.</h1><p class="lead">Start with why the site exists, then inspect the machine-readable record and its limits.</p></div><div class="utility-actions"><a class="primary-button" href="#for-people"><span>Why this exists</span>${arrowIcon()}</a><a class="secondary-button" href="#intent-boundary"><span>See the boundaries</span>${arrowIcon()}</a></div></div></header><div class="narrow prose"><h2 id="for-people">Why this exists</h2><p>AI systems can make confident statements about files, sources, evaluations, identities and external actions. Some are supported. Some conflict with observable state. Some cannot be resolved with the evidence available. Treating all three as the same makes both trust and criticism less useful.</p><h2>How every finding stays inspectable</h2><ul><li>Compare bounded claims with declared evidence.</li><li>Show missingness beside aggregates.</li><li>Bind public claims to exact identities and source revisions.</li><li>Keep consequential action ambiguity visible until state is reconciled.</li><li>State what remains unknown and what the evidence does not prove.</li><li>Invite reproducible, public-safe counterexamples.</li></ul><h2 id="for-machines">For agents and automated readers</h2><p>This is a static, public evidence record rather than a live model or API. Automated readers can use the <a href="../llms.txt">plain-text discovery summary</a>, <a href="../data/deception-cases.v1.json">six-case JSON pack</a>, <a href="../schemas/deception-case-v1.schema.json">JSON Schema</a> and <a href="../data/source-map.v1.json">exact-revision source map</a>. The <a href="../tools/">evidence tools</a> and <a href="../challenge/">challenge route</a> explain how to reproduce or contest a finding.</p><p>Machines can rely on the published classifier relationship, exact case values, reviewed-through dates, licensing and provenance in those files. They should not infer motive, prevalence, live-model behavior or product certification from the records. See the repository’s <a href="${PROJECT_URL}/blob/main/LICENSING.md">licensing explanation</a> and <a href="${PROJECT_URL}/blob/main/NOTICE">provenance notice</a>.</p><h2>AI-assistance disclosure</h2><p>Mike Parsons leads the investigation and is responsible for its public framing. AI assistance was used to help structure, draft, implement and test this repository. The six cases are synthetic. Their deterministic findings are produced by published rules and independently reviewable data rather than a live model.</p><h2 id="intent-boundary">What this is not</h2><p>This is not a claim that every incorrect output is an intentional lie. It is not a deception score, a product ranking, a certification, a consciousness test or a universal account of AI safety. Version one records intent as <strong>not assessed</strong>.</p></div>`,
  });
}
